...
class App extends Component {
  state = {
    ...
    category: 'FRUIT',
    imgUrl: initialImagesList[0].imageUrl,
    score: 0,
    timerValue: 60,
  }
  ...
  onClickCategory = tabId => {
    const updatedImagesList = initialImagesList.filter(
      each => each.category === tabId,
    )
    this.setState({imagesList: updatedImagesList, category: tabId})
    // console.log(updatedImagesList)
  }

  onClickScore = imageUrl => {
    const {imgUrl} = this.state
    const imageLength = initialImagesList.length
    const newImageUrl =
      initialImagesList[Math.ceil(Math.random() * imageLength)].imageUrl
    if (imageUrl === imgUrl) {
      this.setState(prevState => ({
  ...
  render() {
    const {imagesList, category, imgUrl, score, timerValue} = this.state
    const searchedResult = imagesList.filter(each => each.category === category)
    console.log(timerValue)
    if (timerValue === 0) {
      ...
      ele = (
        <>
          <img src={imgUrl} className="match-img" alt="match img" />
        ...
        <nav className="nav-container">
          ...
          <ul className="score-container">
            ...
            <li className="timer-container">
              ...
              />
              <p className="timer-secs">{timerValue} sec</p>
            </li>
          </ul>
        </nav>
        <div className="body-container">{ele}</div>
      </div>
    )
  }
}
